...
export function useCreateTokenMutation() {
  ...
  return useMutation({
    mutationFn: async (param: { createTokenDTO: CreateTokenDTO, authToken: string }): Promise<Token> => {
...
export function useVoidTokenMutation() {
  ...
  return useMutation({
    mutationFn: async (param: { authToken: string, tokenId: string }): Promise<Token> => {
...
export function useClaimTokenMutation() {
  ...
  return useMutation({
    ...
    onSuccess: (response, variables) => {
      queryClient.invalidateQueries({ queryKey: ["tokens", "userStats"] });
    },
  });
}
